...
export default {
  ...
  methods: {
    ...
    updateRate (item, newRate) {
      // Update the rating when the user clicks on a star
      item.rate = newRate
    },
    savePlace (item) {
      // Save the new place when the user clicks Save
      item.place = item.newPlace
      item.editingPlace = false
    },
    saveRate (item) {
      // Save the new rate when the user clicks Save
      item.rate = item.newRate
